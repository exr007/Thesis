...
Modelling was performed using the {\verb pymc3 } \gls{nuts} extension to a \gls{hmc} sampling algorithm \citep{salvatier_probabilistic_2016}. Convergence was interrogated using the $\widehat{R}$ diagnostic factor using the criteria that chains did not converge if $\widehat{R} > 1.01$.
...
The statistics tests were performed on the residual spectrum for various re-binning factors, n. The plots summarising the statistics tests are shown in Fig.~\ref{fig:BiSON_new_symm_stats} for re-binning factors of n = 1, 2, 5, and 10.
...
\begin{gather*}
%
\nu_0 \, \sim \,\mathcal{U}(0.54, 0.56) \> \mu\mathrm{Hz} \\
%
\Gamma \, \sim \,\mathcal{U}(0.00, 0.05)  \> \mu\mathrm{Hz} \\
%
...
\begin{table}[!ht]
	\begin{center}
		...
		\begin{tabular}{l c r}
			...
			{$\nu_0$} & {0.5500$\left(_{-19}^{+19}\right)$} & {$\mu\mathrm{Hz} $}\\
			
			{$\Gamma$} & {0.0052$\left(_{-28}^{+49}\right)$} & {$\mu\mathrm{Hz} $} \\
			
			{$A$} & {27.1$_{-5.9}^{+7.9}$} & {mG} \\
			
			
			{$bgnd$} & {-0.002$_{-0.029}^{+0.059}$} & {$\mathrm{G}^2 \, \mathrm{Hz}^{-1}$} \\	
